...
function templateUpdate(repeating) {
    if (global.config.general.allowStuckPoolKill && cluster.isMaster && lastBlockHashUpdateTime && Date.now() - lastBlockHashUpdateTime > 30*60*1000) {
        console.error("Block height was not updated for half an hour. Check your graftnoded. Exiting...");
    ...
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err === null) {
            ...
            setTimeout(templateUpdate, 250, repeating);
        }
    });
}
...
if (global.config.general.allowStuckPoolKill && fs.existsSync("block_template_is_stuck")) {
   console.error("Stuck block template was detected on previous run. Please fix Daemon and remove block_template_is_stuck file after that. Exiting...");
   process.exit();
}
