...
ROOT_SQL_PASS=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1) #If you have your Own MySQL root password change it then
CURUSER=$(whoami)
...
cd ~
git clone https://github.com/Mojo-LB/nodejs-dero-pool.git nodejs-pool  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
sudo systemctl enable dero
sudo systemctl start dero
